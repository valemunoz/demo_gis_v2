...
var DRAG_LON,DRAG_LAT;
		...
		{
			$j_cm( "#opc_"+DM_div ).css("border","2px solid #FFFFFF");
			$j_cm( "#main_mapa" ).css("margin-top","19%");
			$j_cm("#"+DM_div).slideDown("slow");
		}
		function esconder(DM_div)
		{
			$j_cm( "#opc_"+DM_div ).css("border","0px");
			$j_cm("#"+DM_div).slideUp("fast",function(){
				$j_cm( "#main_mapa" ).css("margin-top","0");
				});
				GIS_filtro=false;	
			GIS_result=false;
			GIS_iso=false;
	...
	function loadData()
	{
		...
		GIS_serv=replaceAll(GIS_serv,"||","");
		 $j_cm("#grilla").load("includes/query.php", 
			{tipo:12, gr_tipo:GIS_tipo, gr_id:GIS_id, gr_serv:GIS_serv} 
				,function(){	
					closeModalWeb();
					$j_cm( "#grilla" ).dialog( "open" );
	...
	function buscar()
{
	...
	var query=$j_cm.trim(document.getElementById("query").value);
	var radio=$j_cm.trim(document.getElementById("radio_fil").value);
	var mis_poli=$j_cm.trim(document.getElementById("mis_poli").value);
	if(query!="" && (radio=="" || $j_cm.isNumeric(radio)))
	{
		limpiarmapa();
		
		if(document.getElementById("dir").checked)//direccion
		{
			$j_cm("#load").show("fast");
			$j_cm("#list").load("includes/query.php", 
			{tipo:1, consulta:encodeURI(query), lon:CM_centro.lon, lat:CM_centro.lat,radio:radio,poli:mis_poli} 
				,function(){	
					...
					try
  				{
    				if(document.getElementById("lug").checked)//direccion
						{
	...
	{
		if(radio!="" || !$j_cm.isNumeric(radio))
		{
			alert("El radio debe ser de valor numerico");
		}else
			{
	...
	function BuscarIso()
	{
		...
		var minutos_iso=$j_cm.trim(document.getElementById("min_iso").value);
		if(minutos_iso!="" && $j_cm.isNumeric(minutos_iso) && minutos_iso<=30 && minutos_iso >0)
		{
			esconder('cuatro');
			cerrar('cuatro');
			$j_cm("#load").show("fast");
  	  ...
  	  $j_cm("#output").load("includes/query.php", 
	...
	function buscaRuta(origin,destino,minutos)
	{
		...
		if(GIS_ultimo==1)
		{
    	  ...
    	  function(result, status) {     
    	  	alert('Busca ' + destino.lat() + ',' + destino.lng() + '');                
    	    if (status == google.maps.DirectionsStatus.OK) {
    	      ...
    	      alert('DirectionsService Status:' + status + ', origin: (' + destino.lat() + ',' + destino.lng() + ')');
    	    }
    	  });
    }
	}
	function closeModalWeb()
{
	 ...
	 $j_cm('#popup_web').fadeOut('slow');
}

function showMensaje(CM_texto)
{
	$j_cm('.cont_web').html(CM_texto);
 $j_cm('#popup_web').fadeIn('fast');
...
function printMapa()
{
   ...
   w=window.open();
	 w.document.write($j_cm('#mapa').html());
	 w.print();
	 w.close();
...
function savePoli(lon,lat)
{
	...
	//alert(vertices);
	$j_cm("#contenido_cuatro").load("includes/query.php", 
...
function getDataManz()
{
  ...
  if($j_cm.isNumeric(radio))  	    
  {
	/*	var body = document.getElementsByTagName("body")[0];
		...
		scr.setAttribute("id","scriptTemporal");
		body.appendChild(scr);*/
		showMensaje("Obteniendo Datos...<br><img src='images/load_central.gif'>");
		$j_cm("#output").load("includes/query.php", 
...
function controlPoligono()
{
	if(CM_polygon_draw)
	{
			...
			esconder('cinco');
			esconder('uno');
		showMensajeTiming("<div id=msg_alert>Control poligono Activado!. Para desactivarlo haz click en el mismo icono</div>",1500);
		$j_cm( "#pd" ).css("border","2px solid #FFFFFF");
	}else
		{
			$j_cm( "#pd" ).css("border","0px");
...
{
	$j_cm("#mis_poli").load("includes/query.php", 
				{tipo:25} 
					,function(){	
...
function activarMarcador(nom_cat,id_cat)
{
	if(!GIS_DRAG)
	{
		...
		$j_cm( "#md" ).css("border","2px solid #FFFFFF");
		showMensaje("Para corregir ubicacion debe mover el marcador sobre el mapa <img height=20px width=20px src='images/iconos/marker2.png'>.<br>Para desactivar el control haga click en el icono <img height=20px width=20px src='images/iconos/marker.png'> de la barra de herramientas");
		GIS_DRAG=true;
		limpiarPuntosDrag();
		var CM_centro= map.getCenter().transform(
        new OpenLayers.Projection("EPSG:900913"), // de WGS 1984
...
function saveMarcador(lon,lat,id_div,id_serv)
{
   ...
   categoria=document.getElementById('id_cat_sel'+id_div).value;
	...
	if($j_cm.trim(nombre)!="" && $j_cm.trim(lon)!="" && $j_cm.trim(lat) !="" && ($j_cm.isNumeric(numero) || $j_cm.trim(numero)==""))
	{
				...
				{tipo:27,lat:lat, lon:lon, detalle:detalle,categ:categoria,nom:nombre,detalle:detalle,calle:calle,numero:numero,comuna:comuna,id_ser:id_serv} 
					,function(){	
						if(id_serv == 0)
						{
  						...
  						{
  							$j_cm("#list_puntos").load("includes/query.php", 
...
function loadMisPuntos()
{
	...
	$j_cm("#contenido_cuatrob").load("includes/query.php", 
				{tipo:28} 
					,function(){	
											closeModalWeb();
					}
...
{
	showMensaje("<img src='images/load_central.gif'>");
	$j_cm("#ptoEdit_"+id_div).load("includes/query.php", 
					{tipo:32,id_div:id_div,id_serv:id_serv} 
						,function(){	
							$j_cm("#ptoEdit_"+id_div).toggle("slow");
							closeModalWeb();
						}
					);  
...
function LoadFileCateg()
{
	...
	if (typeof $j_cm("#i_file")[0].files[0] != "undefined" && cat > 0)
  {
  	var file = $j_cm("#i_file")[0].files[0];
    //obtenemos el nombre del archivo
    var fileName = file.name;
    var ext=fileName.split(".");
    ext=ext[1];
    sizea=file.size;    
    var formData = new FormData($j_cm(".formulario")[0]);
    if(ext=="txt" || ext=="TXT")
    {
    			$j_cm.ajax({
