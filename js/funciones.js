...
function loadServicios()
{
			...
			$j_cm("#centro").load("includes/query.php", 
			{tipo:3} 
				,function(){	
				}
		);
...
function loadAntenas()
{
	...
	var AM_exten = getExtencion();
			...
			/*$j_cm("#resultado_seccion").load("includes/query.php", 
  ...
  $("#radio").show();
	var CM_centro= map.getCenter().transform(
        ...
        new OpenLayers.Projection("EPSG:4326") // a Proyección Esférica Mercator
			...
			$j_cm("#resul_radio").load("includes/query.php", 
...
function displayDataCluster(event) {
            ...
            function loadServiciosPoligono()
            {
            	...
            	//.transform(new OpenLayers.Projection("EPSG:900913"), new OpenLayers.Projection("EPSG:4326"));
							//alert(vert);
            	$j_cm("#resul_poligono").load("includes/query.php", 
								{tipo:10, coord: ''+vert+''} 
								,function(){	
							});
							}catch(e){}   
...
function viabilidadAntena()
{
	...
	$j_cm("#output").load("includes/query.php", 
 	...
 	{
 		showMensaje("La altura debe ser mayor a 12 metros de altura.");
 	}
}
