...
$CM_path_base="http://localhost/demos_moxup/demo_gis_v2";
...
if($_REQUEST['tipo']==1)
{
	...
	if(count($data)>0)
	{
		...
		foreach($data as $dat)
		{
 		  ...
 		  <li class="lista">
					<img src="img/iconos/direccion.png" class="img_list"><span class="tit_1"> <a class="link_dir" href="javascript:addMarcador('img/iconos/direccion.png','35,35',<?=$dat[6]?>,<?=$dat[7]?>,'');moverCentro(<?=$dat[6]?>,<?=$dat[7]?>,16);"><?=$dat[1]?> # <?=$dat[3]?>, <?=$dat[4]?></a></span><br>					
	...
	if(count($data)>0)
	{
		...
		foreach($data as $dat)
		{
 		  ...
 		  <li class="lista">
					<img src="img/iconos/direccion.png" class="img_list"><span class="tit_1"> <a class="link_dir" href="javascript:addMarcador('img/iconos/direccion.png','35,35',<?=$dat[1]?>,<?=$dat[0]?>,'');moverCentro(<?=$dat[1]?>,<?=$dat[0]?>,16);"><?=$dat[3]?> # <?=$dat[2]?>, <?=$dat[4]?></a></span><br>					
					<!--span class="tit_2">Titulo2</span><br>
					<span class="tit_3">Titulo3</span><br-->
				</li>
	...
	}else//buscamos en google
	{
		...
		foreach($data as $dat)
		{
			?>
					...
					<li class="lista">
					<img src="img/iconos/direccion.png" class="img_list"><span class="tit_1"> <a class="link_dir" href="javascript:addMarcador('img/iconos/direccion.png','35,35',<?=$dat[7]?>,<?=$dat[8]?>,'');moverCentro(<?=$dat[7]?>,<?=$dat[8]?>,16);"><?=$dat[3]?> # <?=$dat[2]?>, <?=$dat[4]?></a></span><br>					
					<!--span class="tit_2">Titulo2</span><br>
					<span class="tit_3">Titulo3</span><br-->
				</li>
			<?php
...
}elseif($_REQUEST['tipo']==2) //antenas
{
	...
	?>
		...
		addMarcadorServicio('img/iconos/antena.png','50,50',<?=$dat[1]?>,<?=$dat[2]?>,'<strong>ANTENA TELEFONIA</strong> <br> Empresa: <?=$dat[0]?>');			
...
{
	?>
	<!--img class="img_control_off" id="manz" name="manz" title="Activar Manzanas" src="img/iconos/manzanas.png" onclick="accionManzana();"-->
	<img class="img_control_off" name="manz" id="manz" title="Activar Manzanas" src="img/iconos/manzana.png" height=32px class="menu_boton" onclick="javascript:accionManzana();">
	
	<?php
	if(in_array("4", $_SESSION['us_apps']))
	{
		?>
		<img class="img_control_off" name="clus" id="clus" title="Cluster ABC1" src="img/iconos/cluster.png" height=32px class="menu_boton" onclick="javascript:loadDataCluster();">
		<?php
	}
	?>
	...
	{
		?>
		<img class="img_control_off" name="hm_2" id="hm_2" title="HeatMap ABC1" src="img/iconos/es_abc.png" height=32px class="menu_boton" onclick="javascript:heatMap2('abc1');">
		<?php
	}
	?>
	...
	{
		?>
		<img class="img_control_off" name="hm" id="hm" title="HeatMap E" src="img/iconos/es_e.png" height=32px class="menu_boton" onclick="javascript:heatMap2('e');">
		<?php
	}
	...
	{
		?>
		<img class="img_control_off" name="clus_antena" id="clus_antena" title="Cluster Antenas" src="img/iconos/antena.png" height=32px class="menu_boton" onclick="javascript:loadDataClusterAntenas();">
		<?php
	}
	if(in_array("9", $_SESSION['us_apps']))
	{
		?>
		<img class="img_control_off" name="heat_antena" id="heat_antena" title="Heatmap Antenas" src="img/iconos/antena.png" height=32px class="menu_boton" onclick="javascript:heatMapAntena();">
		<?php
	}
	...
	?>
	<img class="img_control_off" name="img_0" id="img_0" title="Activar Bancos" src="img/iconos/banco.png" height=32px class="menu_boton" onclick="checkServ(4,'img/iconos/banco.png',0);">
	<img class="img_control_off" name="img_1" id="img_1" title="Activar Colegios" src="img/iconos/universidad.png" height=32px class="menu_boton" onclick="checkServ(1,'img/iconos/universidad.png',1);">
	<img class="img_control_off" name="img_2" id="img_2" title="Activar Supermercados" src="img/iconos/supermercado.png" height=32px class="menu_boton" onclick="checkServ(11,'img/iconos/supermercado.png',2);">
  	<script>
  		addServicios('Bancos',0,4,'img/iconos/banco.png');
  		addServicios('Colegios',1,1,'img/iconos/universidad.png');
  		addServicios('Supermercados',2,11,'img/iconos/supermercado.png');
  	</script>
	<?php
...
}elseif($_REQUEST['tipo']==7)// servicios por radio
{
	...
	<ul>
		...
		foreach($data as $i => $dat)
		{
			...
			?>
			<script>
				addMarcadorNativo("img/iconos/circle.png","13,13",<?=$dat[3]?>,<?=$dat[4]?>,"<b><?=ucwords($dat[1])?></b>");
				</script>
...
}elseif($_REQUEST['tipo']==10)//cluster
{
	...
	foreach($data as $i => $dat)
	{
		?>
			...
			<script>
				addMarcadorNativo("img/iconos/circle.png","13,13",<?=$dat[1]?>,<?=$dat[2]?>,"<b><?=ucwords($dat[0])?></b>");
			</script>
...
elseif($_REQUEST['tipo']==11)// Lugares
{
	...
	//print_r($data);
	?>
		<br><div id="results">Lugares Encontrados: <?=count($data);?></div>
		<ul id="list">			
		<?php
		foreach($data as $dat)
		{
			...
			if($distancia or is_numeric($_REQUEST['poli']))
			{
				$texto.="<div id=pop_titulo3>Distancia: ".round($dat[5]/1000,2)."Km</div>";
					...
					?>
					<script>
						addMarcadorNativo('img/iconos/place.png','35,35',<?=$dat[3]?>,<?=$dat[4]?>,'<?=$texto?>');
						</Script>
					<?php
			}
			if(trim($dat[9])!="")
 		  ...
 		  <li class="lista">
 		  	...
 		  	if(!$distancia and !is_numeric($_REQUEST['poli']))
				{
				?>
					<img src="img/iconos/place.png" class="img_list"><span class="tit_1"> <a class="link_dir" href="javascript:addMarcadorNativo('img/iconos/place.png','35,35',<?=$dat[3]?>,<?=$dat[4]?>,'<?=$texto?>');moverCentro(<?=$dat[3]?>,<?=$dat[4]?>,16);"><?=ucwords($dat[1])?>, <?=ucwords($dat[8])?> <?=$text_dis?></a></span><br>					
				<?php
				}else
				{
					?>
					<img src="img/iconos/place.png" class="img_list"><span class="tit_1"> <a class="link_dir" href="javascript:moverCentro(<?=$dat[3]?>,<?=$dat[4]?>,17);popNativo('<?=$texto?>',0,<?=$dat[4]?>,<?=$dat[3]?>);"><?=ucwords($dat[1])?>, <?=ucwords($dat[8])?> <?=$text_dis?></a></span><br>					
					<?php
				}
				?>		
					<!--span class="tit_2">Titulo2</span><br>
...
}elseif($_REQUEST['tipo']==12)// load grilla resultados
{
	...
	<div id="header_grilla">
		...
		<img src="img/down.png" title="Descargar CSV" onclick="javascript:window.location='csv/<?=$nom_archivo?>'" class="menu_boton">
	</div>
	<br>
...
}elseif($_REQUEST['tipo']==13)
{
	...
	*/
	...
	<script>
    	...
    	GIS_origin = new google.maps.LatLng(CM_centro.lat,CM_centro.lon);
			addMarcadorServicio('img/iconos/place.png','30,30',CM_centro.lat,CM_centro.lon,'');	
  ...
  style_blue.fillOpacity=0.7;	 
  style_blue.stroke=false;*/
  //isoGoogle("<?=$arr_ll?>",<?=$_REQUEST['minutos']?>);
  //addPoligono("<?=$poli?>","",style_blue);
	//extendPoligonos();
		</script>
	<?php
...
}elseif($_REQUEST['tipo']==21)//algoritmo antena
{
	...
	if(count($data)>0)
	{
		...
		<script>
			//closeModalWeb();
			esconder('cinco');
			showMensajeestatico("Se encontraron <?=count($data)?> antenas en un radio de 50mts");
			</Script>
		<?php
		foreach($data as $antena)
		{
			...
			?>
			<SCRIPT>
				addMarcadorNativo("img/iconos/antena2.png","30,30",<?=$antena[2]?>,<?=$antena[3]?>,"<?=$html?>");
			</script>
			<?PHP
	...
	}else
	{
		...
		<script>
			//closeModalWeb();
			showMensajeestatico("No se encontraron antenas en un radio de 50mts");
			</Script>
		<?php
	}
...
}elseif($_REQUEST['tipo']==22)//algoritmo antena2
{
	...
	if(count($servicios)>0)
	{
		...
		<script>
			//closeModalWeb();
			esconder('cinco');
			showMensajeestatico("Se encontraron <?=count($data)?> antenas en un radio de <?=$radio?>mts <br> y es una zona sensible con <?=count($servicios)?> puntos.");
			</Script>
		<?php
...
}elseif($_REQUEST['tipo']==23) //manzanas busqueda radio
{
  ...
  $texto_html="<div id=pop_titulo1>DATOS</div>";
	...
	<script>
		...
		DibujarCirculoAdap("#5C3DDC","#5C3DDC",<?=$radio*3/100?>,<?=$_REQUEST['lon']?>,<?=$_REQUEST['lat']?>,1);
		map.zoomToExtent(vectorCirculo.getDataExtent(),false);
		addMarcadorNativo("img/charts.ico","24,24",<?=$_REQUEST['lat']?>,<?=$_REQUEST['lon']?>,"<b><?=$texto_html?></b>");		
	</script>
	<?php
...
}elseif($_REQUEST['tipo']==28)
{
	...
	Categoria :<select class=input_form id="cat_pto" name="cat_pto">
...
}elseif($_REQUEST['tipo']==30)
{
	if($_REQUEST['tip']==1)
	{
		...
		}else
		{
			?>
			<script>	
				showMensajeestatico("<div id=msg_alert>Categoria <?=strtoupper($_REQUEST['nombre'])?> ya existe, por favor modifique los datos ingresados.</div>");
